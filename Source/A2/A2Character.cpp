...
AA2Character::AA2Character()
{
	...
	/*---------------------SETTING UP DAMIAN'S VARIABLES---------------------------*/

	CurrentPickup = NULL;
	PickupFound = false;
	PickupName = NULL;
	PickupDisplayText = NULL;
	bInteract = false;
}
...
void AA2Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//BINDING MY INTERACT EVENT
	PlayerInputComponent->BindAction("Interact", IE_Pressed, this, &AA2Character::Interact);
}
	...
	if (ProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<AA2Projectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
				...
				World->SpawnActor<AA2Projectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
...

{
	...
	// #### DEBUGGING only. Do *not* do this in a real game, the framerate hit will kill you
	...
	//	GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Yellow, TEXT("Debug text"));
...

	{
		...
		if (GEngine)
			...
			GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Yellow, PickupDisplayText);
...
void AA2Character::ClearPickupInfo()
{
	 CurrentPickup = NULL;
	 PickupFound = NULL;
	 PickupDisplayText = NULL;
...
/*void AA2Character::ManagePickup(APickup* DetectedPickup)
{
...
/*void AA2Character::SwitchLogic(UWorld* World, ADoor* SwitchDoor, ASwitch* Switch)
{
	TArray <AActor> LevelActors;
	GetLevel()->Actors;

	for (int i = 0; i < GetLevel()->Actors; i++)
	{
	...
	SwitchDoor->OpenDoor();
	Switch->setSwitchIsOn(true);
}
*/

void AA2Character::Interact()
{
	bInteract = true;
}